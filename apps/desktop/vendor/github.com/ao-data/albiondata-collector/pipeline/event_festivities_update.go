...
func (event eventFestivitiesUpdate) Process(state *albionState) {
	log.Println("Got festivities update event...")
	...
	if err != nil {
		log.Printf("Could not parse festivities update: %v", err)
		return
	}
	...
	log.Printf("Sending %d festivities to ingest (Identifier: %s)", len(upload.Events), identifier)
