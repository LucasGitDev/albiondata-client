...
func createUploadersWithToken(targets []string, token string) []uploader {
	...
	for _, target := range targets {
		...
		if len(target) < 4 {
			log.Printf("Got an ingest target that was less than 4 characters, not a valid ingest target: %v", target)
			continue
		}
		...
		if target[0:8] == "http+pow" || target[0:9] == "https+pow" {
			...
			log.Printf("An invalid ingest target was specified: %v", target)
		}
	}
...
func sendMsgToPublicUploaders(upload interface{}, topic string, state *albionState, identifier string) {
	...
	if err != nil {
		log.Printf("Error while marshalling payload for %v: %v", err, topic)
		return
	}
...
func sendMsgToPrivateUploaders(upload lib.PersonalizedUpload, topic string, state *albionState, identifier string) {
	if ConfigGlobal.DisableUpload {
		log.Println("Upload is disabled.")
		return
	}
	...
	// 	log.Println("The player name or id has not been set. Please restart the game and make sure the client is running.")
	// 	log.Println("The player name or id has not been set. Please restart the game and make sure the client is running.")
	// 	return
	// }
