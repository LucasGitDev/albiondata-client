...
func (state albionState) IsValidLocation() bool {
	...
	case state.LocationId == "":
		log.Println("The players location has not yet been set. Please transition zones so the location can be identified.")
		return false
	...
	case strings.HasSuffix(state.LocationId, "-Auction2"):
		return true
	default:
		log.Println("The players location is not valid. Please transition zones so the location can be fixed.")
		return false
	}
}
...
func (state albionState) GetServer() (int, string) {
	...
	if isAlbionIP {
		log.Printf("Returning Server ID %v (ip src: %v)", serverID, state.GameServerIP)
		log.Printf("Returning AODataIngestBaseURL %v (ip src: %v)", AODataIngestBaseURL, state.GameServerIP)
	}
