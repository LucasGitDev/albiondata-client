...
func (op operationAuctionGetItemAverageStats) Process(state *albionState) {
	...
	log.Printf("Market History - Caching %d at %d.", mhInfo.albionId, index)
}
...
func (op operationAuctionGetItemAverageStatsResponse) Process(state *albionState) {
	...
	if state.marketHistoryIDLookup[index].albionId < 1 {
		log.Printf("Market History - Market history at index %d is invalid. Has albionId: %d ", index, state.marketHistoryIDLookup[index].albionId)
		return
	}
	...
	log.Printf("Market History - Loaded itemID %d from cache at index %d", mhInfo.albionId, index)
	log.Println("Got response to GetItemAverageStats operation for the itemID[", mhInfo.albionId, "] of quality: ", mhInfo.quality, " and on the timescale: ", mhInfo.timescale)
	...
	for i := range op.ItemAmounts {
		...
		if op.ItemAmounts[i] < 0 {
			if op.ItemAmounts[i] < -124 {
				...
				log.Printf("Market History - Ignoring negative item amount %d for %d silver on %d", op.ItemAmounts[i], op.SilverAmounts[i], op.Timestamps[i])
				continue
			}
			// however these can be interpreted by adding them to 256
			// TODO: make more sense of this, (perhaps there is a better way)
			log.Printf("Market History - Interpreting negative item amount %d as %d for %d silver on %d", op.ItemAmounts[i], 256+op.ItemAmounts[i], op.SilverAmounts[i], op.Timestamps[i])
	...
	if len(histories) < 1 {
		log.Print("Auction Stats Response - no history")
		return
	}
	...
	log.Printf("Sending %d market history item average stats to ingest for albionID %d (Identifier: %s)", len(histories), mhInfo.albionId, identifier)
