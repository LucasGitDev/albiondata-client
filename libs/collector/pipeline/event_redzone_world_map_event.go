...
func (event eventRedZoneWorldMapEvent) Process(state *albionState) {
	log.Println("Got red zone world event...")
	...
	if state.BanditEventLastTimeSubmitted.IsZero() || time.Since(state.BanditEventLastTimeSubmitted).Seconds() >= 60 {
		...
		log.Printf("Bandit Event detected (Phase: %d) ending at %d", event.Phase, event.EventTime)
		...
		log.Printf("Sending bandit event to ingest (Identifier: %s)", identifier)
