...
func (op operationReadMail) Process(state *albionState) {
	log.Println("Got ReadMail operation...")
	...
	if mailInfo == nil {
		log.Println("Mail Info is not valid. Please transition zones or click at notification, so the mails can be loaded.")
		return
	}

	if mailInfo.OrderType == "MARKETPLACE_SELLORDER_FINISHED_SUMMARY" {
		log.Println("Read finished sell order.")
		notification = decodeSellNotification(op, body)

	} else if mailInfo.OrderType == "MARKETPLACE_SELLORDER_EXPIRED_SUMMARY" {
		log.Println("Read expired sell order.")
...
func decodeSellNotification(op operationReadMail, body []string) lib.MarketNotification {
	...
	if err != nil {
		log.Println("Could not parse amount in market sell notification ", err)
		return nil
	}

	price, err := strconv.Atoi(body[3])
	if err != nil {
		log.Println("Could not parse price in market sell notification ", err)
		return nil
	}
