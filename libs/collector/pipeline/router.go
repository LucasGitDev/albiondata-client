...
func (r *Router) run() {
	...
	if ConfigGlobal.RecordPath != "" {
		...
		if err != nil {
			log.Println("Could not open commands output file ", err)
		} else {
	...
	for {
		select {
		case <-r.quit:
			log.Println("Closing router...")
			if file != nil {
				err := file.Close()
				if err != nil {
					log.Println("Could not close commands output file ", err)
				}
			}
			return
		...
		case raw := <-r.recordRawPacket:
			if encoder != nil {
				...
				if err != nil {
					log.Println("Could not encode raw packet ", err)
				}
			}
		}
	}
